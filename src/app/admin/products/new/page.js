...
export default function NewProductPage() {
  ...
  const addVariant = () => {
    setVariants([...variants, {
      ...
      color: "",
      priceOverride: "",
      inventory: {} // warehouseId -> quantity
    }]);
  };
  ...
  const handleSave = async () => {
    try {
      // Map variants for payload
      const mappedVariants = variants.map(v => ({
        sku: v.sku,
        priceOverride: Number(v.priceOverride) || null,
        attributes: { size: v.size, color: v.color },
        inventoryData: v.inventory // custom field we will process in API
      }));
    ...
    <div className="max-w-5xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <h2 className="font-display text-2xl">Create New Product</h2>
        </div>
        <Button onClick={handleSave} className="gap-2 bg-foreground text-background"><Save size={16} /> Save Product</Button>
      </div>
